...
{users.map(user => (
  <tr key={user.id}>
    ...
    <td>{user.phone}</td>
    <td>
    <Button color="gray" onClick={() => handleEditUser(user.id)}>Edit</Button>
    <Button color="gray" onClick={() => deleteUser(user.id)}>Delete</Button>
  </td>
  </tr>
    ))}
  </tbody>
...
<Button color="primary" onClick={toggleModal}>Add User</Button>
  <Modal isOpen={modalOpen} toggle={toggleModal}>
    ...
    <ModalFooter>
      <Button onClick={() => { toggleModal(); resetForm(); }}>Cancel</Button>
      <Button onClick={handleSaveUser}>{editMode ? 'Update' : 'Save'}</Button>
    </ModalFooter>
